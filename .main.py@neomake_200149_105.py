...
inputGRE = form.text_input("Masukan GRE Score: ", 0)
inputTOEFL = form.text_input("Masukan TOEFL Score: ", 0)
inputUnivRating = form.text_input("Masukan Rating Univ: ", 0)
inputSOP = form.text_input("Masukan Kekuatan SOP: ", 0)
inputLOR = form.text_input("Masukan Kekuatan LOR: ", 0)
inputCGPA = form.text_input("Masukan CGPA: ", 0)
inputResearch = form.text_input("Pengalaman Researc, 1 Jika Pernah Riset, 0 Jika Tidak", 0)
submit = form.form_submit_button('Submit')

completeData = np.array([[float(inputGRE), float(inputTOEFL), float(inputUnivRating), float(inputSOP), float(inputLOR), float(inputCGPA), float(inputResearch)]])
scaledData = ss_train_test.fit_transform(completeData)

st.write(completeData)
st.write(scaledData)
